...
// Query the database
$query = "SELECT * FROM public.\"Webhooks\" where \"merchantAccount\" in (:ECOMACCOUNT,:POSACCOUNT) ORDER BY \"CreatedDate\" DESC";

try {
    $stmt = $pdo->prepare($query);
    // Bind the JSON data to the parameter
    $stmt->bindParam(':ECOMACCOUNT', $data["ecomMerchantAccount"], PDO::PARAM_STR);
    $stmt->bindParam(':POSACCOUNT', $data["posMerchantAccount"], PDO::PARAM_STR);
    // Execute the query
    $stmt->execute();
