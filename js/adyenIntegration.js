$(document).ready(function () {
    ...
    // Get redirectResult from URL
    const redirectResult = getQueryParam('redirectResult');
    console.log("redirecresult",redirectResult);
    if (redirectResult) {
    ...
    console.log('Payment Details request Body', JSON.parse(raw)); // parse to display cleanly
...
//done
    ...
    function initAdyen() {
        ...
        if (selectedIntegration == 'ps' || !selectedIntegration) {
            return;
        }
        if (selectedIntegration == 'paybylink') {
            ...
            payByLinkRequestBody.amount.value = total.toFixed(0);
            payByLinkRequestBody.amount.currency = currency;
            payByLinkRequestBody.countryCode = country;
            payByLinkRequestBody.merchantAccount = merchantAccount;

            $('#adyen-container').html('');

            raw = JSON.stringify(payByLinkRequestBody);

            console.log('PAy By Link Request Body', $.parseJSON(raw));
            var headersSent = {
                'X-API-Key': JSON.parse(sessionStorage.connectionConfig).configuration.apiKeyECOM
            }
            callAPI('./../utils/paybylink.php', raw, headersSent, function (response) {
                console.log('Pay by link Response', response);
                $("#adyen-container").qrcode({
                    text: response.url,
                    size: 250,

                });
                $(`<span class="info-text-qrcode"><h4> Please scan the QR Code to go to to <a href="${response.url}">checkout page</a> </h4></span>`).appendTo('#adyen-container');
    ...
    configuration = {
        ...
        onSubmit: (state, component) => {
            ...
            paymentRequestBody.amount.currency = currency.toUpperCase();
            ...
            // paymentRequestBody.paymentMethod.brand = "visa"
    ...
    async function LoadAdyen(data, integrationType) {
        ...
        if (integrationType == 'saved-card') {

            integrationType = 'card'

            if (data.storedPaymentMethods.length == 0) {
                dropinComponent = checkout.create(integrationType, dropInConfig);

            } else {
                dropinComponent = checkout.create(integrationType, checkout.paymentMethodsResponse.storedPaymentMethods[0]);
    ...
    $('.toggle-billing-address-input input').on('change', function () {
        ...
        if ($(this).is(':checked')) {
            ...
            // configuration.paymentMethodsConfiguration.card.billingAddressMode = "partial";
            configuration.paymentMethodsConfiguration.card.billingAddressRequiredFields= ['postalCode', 'country'];
            // billingAddressRequiredFields: ['postalCode', 'country']
            // configuration.paymentMethodsConfiguration.card.data.country = "Country"
